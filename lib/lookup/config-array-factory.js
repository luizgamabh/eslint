...
class ConfigArrayFactory {
    ...
    _normalizeConfigData(configData, filePath, name) {
        ...
        if (Array.isArray(configData)) {
            return this._normalizeArrayConfigData(configData, filePath, name);
        }
        return this._normalizeObjectConfigData(configData, filePath, name);
    }

    *_normalizeArrayConfigData(configDataArray, filePath, name) {
        let i = 0;
        ...
        for (const configData of configDataArray) {
            ...
            if (typeof configData === "string") {
                yield* this._loadExtends(configData, filePath, elementName);
            } else if (Array.isArray(configData)) {
                yield* this._normalizeArrayConfigData(
                    configData,
                    filePath,
                    elementName
                );
    ...
    *_normalizeObjectConfigDataBody(
    ...
    _loadPlugins(names, importerPath, importerName) {
        ...
        return Object.entries(names).reduce((map, [id, name]) => {
            const plugin = this._loadPlugin(name, importerPath, importerName);
