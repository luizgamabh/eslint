...
function mergePlugins(target, source) {
    ...
    for (const key of Object.keys(source)) {
        ...
        } else if (key !== target[key].id || key !== source[key].id) {
            throw new Error(`Plugin "${key}" was conflicted between "${source[key].importerName}" and "${target[key].importerName}"`);
        }
    }
}
